...
| extra | Dummy-node chains for edges that span more than one layer | Insert one placeholder node per intermediate layer for every edge that skips a layer; treat it like a real node for ordering and coordinates, then reconnect for drawing | Sugiyama et al. 1981 (this is the mechanism that makes phases 3 and 4 work at all for non-adjacent edges) | 20-30 lines |
